...
if [ "${enable_dns}" == "0" ]; then
	...
	if grep -q "^dns:" "$RUNTIME_YAML"; then
		sed -i "/dns:/i\#clash-openwrt" "$RUNTIME_YAML" 2>/dev/null
		sed -i "/#clash-openwrt/a\#=============" "$RUNTIME_YAML" 2>/dev/null
		sed -i '1,/#clash-openwrt/d' "$RUNTIME_YAML" 2>/dev/null
		sed -i '/#=============/ d' "$RUNTIME_YAML" 2>/dev/null
	fi
	cat "$RUNTIME_YAML" >>$TEMP_FILE 2>/dev/null
	mv $TEMP_FILE "$RUNTIME_YAML" 2>/dev/null

elif [ "${enable_dns}" == "1" ]; then

	if grep -q "^proxies:" "$RUNTIME_YAML"; then
		sed -i "/^proxies:/i\#clash-openwrt" "$RUNTIME_YAML" 2>/dev/null
	elif grep -q "^proxy-providers:" "$RUNTIME_YAML"; then
		sed -i "/proxy-providers:/i\#clash-openwrt" "$RUNTIME_YAML" 2>/dev/null
	fi

	sed -i "/#clash-openwrt/a\#=============" "$RUNTIME_YAML" 2>/dev/null
	sed -i "/#=============/a\ " "$RUNTIME_YAML" 2>/dev/null
	sed -i '1,/#clash-openwrt/d' "$RUNTIME_YAML" 2>/dev/null

	mv /etc/clash/config.yaml /etc/clash/dns.yaml
	cat $TEMP_FILE /etc/clash/dns.yaml >"$RUNTIME_YAML" 2>/dev/null
...
if [ "${fake_ip}" == "fake-ip" ]; then
	...
	if [ -s "$CUSTOM_FILE" ]; then
		...
		while [[ $count_num -le $num ]]; do
			...
			if ! echo "$line" | grep -q '^ \{0,\}#' 2>/dev/null; then
	...
	elif grep -qE '^ {0,}enhanced-mode:' "/etc/clash/config.yaml"; then
