...
CURL_GROUP_CACHE="/usr/share/clash/backup/clash_gorup.json"
CURL_NOW_CACHE="/usr/share/clash/backup/clash_now.json"
CURL_CACHE="/usr/share/clash/backup/clash_curl.json"
HISTORY_PATH="/usr/share/clash/backup/history"
...
if [ "$(sed -n '$p' "$CURL_CACHE" 2>/dev/null)" = "200" ]; then
	if [ ! -d /usr/share/clash/backup ];then
		mkdir -p /usr/share/clash/backup 2>/dev/null
	fi
...
rm -rf /usr/share/clash/backup/clash_*.json  2>/dev/null 
